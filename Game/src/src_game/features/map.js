...
export class Map extends Phaser.Scene {
  ...
  create() {
    ...
    this.network.myEventHandler = (code, content, actorNr) => {
      if (code === MessageTypes.PLAYER_MOVE) this.updateOtherPlayer(actorNr, content);
    };
    ...
    // --- FLAG ---
    this.flag = this.physics.add.sprite(300, 300, 'redflag').setScale(0.5);
    ...
    this.generateMapGraphics(testMap);

    // --- CAMERA ---
    const cam = this.cameras.main;
    cam.setBounds(0, 0, 2560, 1440);
    cam.startFollow(this.player);
    cam.setZoom(4); // Adjust zoom
  ...
  generateMapGraphics(testMap) {
    ...
    testMap.features.forEach(f => {
      ...
      coordsArray.forEach(([lon, lat]) => { lons.push(lon); lats.push(lat); });
    });
    ...
    if (mapAspect > screenAspect) { scaleX = canvasWidth / lonRange; scaleY = scaleX; offsetX = 0; offsetY = (canvasHeight - latRange * scaleY)/2; }
    else { scaleY = canvasHeight / latRange; scaleX = scaleY; offsetY = 0; offsetX = (canvasWidth - lonRange * scaleX)/2; }
  ...
  updateOtherPlayer(actorNr, content) {
    if (!this.otherPlayers[actorNr]) this.otherPlayers[actorNr] = new Player(this, content.x, content.y, 'BlueStill', null);
    else { const other = this.otherPlayers[actorNr]; other.x = content.x; other.y = content.y; }
  }

  update() {
    if (this.player) this.player.update();
    if (this.player2) this.player2.update();
    if (this.flagHolder) { this.flag.x = this.flagHolder.x; this.flag.y = this.flagHolder.y; }

    if (this.network && this.player) this.network.setPlayerState(this.player.x, this.player.y);
  }
}
